...
|정의|▪ ARIMA(p,d,q) 모형은 d차 차분한 데이터에 위 AR(p) 모형과 MA(q) 모형을 합친 모형<br><br>▪ 주어진 문제를 상호 보완적으로 시계열을 예측하는 하나의 접근 방법|▪ 가장 최근의 일자에 가장 큰 가중치를 둬서 최근의 시장 분위기를 잘 반영<br><br>▪ 전 기간의 데이터를 분석 대상으로 <br>함으로써 가중이동평균에서 문제되는 <br>특정 기간의 데이터만을 분석 대상으로 한다는 단점 보완| ▪ 강한 계절 효과가 있는 시계열과 여러 계절의 기록 데이터에서 가장 잘 작동<br><br>▪ 누락된 데이터와 추세의 변화에 강력하며 일반적으로 이상 값을 잘 처리|
