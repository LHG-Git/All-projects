...
|기술|설명|
|:------:|------|
|CNN|>▪ 영상 인식에 많이 사용되는 신경망으로 이미치 처리에 특히 적합한 신경망<br>▪ 행렬 연산량을 줄이기 위해 곱하고 더하는 행렬 연산으로 데이터를 줄이고 인접 행렬값 중 큰 값을 선택하는 기법 등을 활용한 성능 개선이 특징<br>▪행렬 연산을 반복하면서 이미지, 동영상은 요약되며 전체 형상을 파악할 수 있는 형태로 요약, 변환|
|시각 지능|▪ 사람이 사물을 인지하고 시공간적으로 상황을 파악할 수 있는 능력<br>▪직관적으로 사물을 인식하는 능력과 심층적 사고에 의한 인지 능력으로 나뉨<br>▪시각지능 딥러닝 기술로 가장 많이 사용하는 컨볼루션 뉴럴 네트워크는 이미지, 동영상을 입력하면 특징을 학습하여 데이터를 분류하는 딥러닝 기술임|
|지능형 CCTV|▪ 지능형CCTV는 CNN+시각지능의 기술을 합친 것<br>▪ 지능형CCTV에서 요구되는 기술은 이미지 분류가 아닌 객체 검출 기술임, 움직이는 영상 내에서 복수로 존재하는 인물과 사물을 찾고 그들의 위치와 관계성을 인식할 수 있어야 함<br>▪ 현재 사용화 되고 있는 지능형 CCTV들은 촬영되고 있는 영상을 분석해 이상 상황을 탐지하고, 문제가 발생했다고 판단될 경우 즉각 관제요원에게 신호를 보내며, 지능형CCTV를 도입하면 적은 인원으로도 많은 CCTV를 동시에 감시할 수 있어 업무 효울이 크게 증가함|
# 📊 EDA(탐색적 데이터 분석)
