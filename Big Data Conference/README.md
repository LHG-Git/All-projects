...
<br>
<h3 align="center"><img src="https://github.com/heegu0513/project/assets/100845169/45770f12-979c-4be5-8c73-a5b1cfd488a7" height = 600px></h3>
...
|데이터|변수 산출|변수|
|------|:-------------:|:--:|
|시간별_공급량 데이터|연월일 컬럼을 datetime 형식으로 변환|년,월,일|
|구분, month, day, 시간|평균값 계산|공급량 평균|
|구분, month, 요일별, 시간별|평균값 계산|공급량 평균|
|공급량|전년도 계산|전년도 공급량|
|계절|겨울 추출|겨울 여부|
|시간|특정 시간 추출|특정 시간|
|요일|평일 추출|평일 여부|
|휴일, 주말|휴일, 주말 각각 추출|휴일, 주말 여부|
|공급사|숫자형 변환|공급사|
|Feature 추가|데이터 수집|가스기름상대가격|
|Feature 추가|데이터 수집|가스전기상대가격|
|Feature 추가|데이터 수집|전산업생산지수|
|Feature 추가|데이터 수집|가스수요산업지수|
|Feature 추가|데이터 수집|기온|
|Feature 추가|데이터 수집|미세먼지|
|Feature 추가|데이터 수집|수입물량지수|
|Feature 추가|데이터 수집|수입금액지수|
|Feature 추가|데이터 수집|수입물가지수(원화)|
|Feature 추가|데이터 수집|에너지소비량|
|Feature 추가|데이터 수집|전력량|
